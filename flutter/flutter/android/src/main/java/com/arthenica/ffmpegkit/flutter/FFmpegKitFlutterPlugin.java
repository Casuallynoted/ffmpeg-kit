...
import io.flutter.plugin.common.PluginRegistry; // Keep for ActivityResultListener IF needed, though new API preferred

// --- V2 EMBEDDING CHANGE ---
// Implements FlutterPlugin and ActivityAware for V2 Embedding registration.
public class FFmpegKitFlutterPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, EventChannel.StreamHandler, PluginRegistry.ActivityResultListener {
    ...
    // Define keys... (Keep all these constants)
    ...
    // --- V2 EMBEDDING CHANGE ---
    // Instance variables moved here from init() or needed by new lifecycle methods
    ...
    private Result lastInitiatedIntentResult; // For ActivityResult
    ...
    // --- V2 EMBEDDING CHANGE: registerWith removed ---
    /*
    @SuppressWarnings("deprecation")
    public static void registerWith(final io.flutter.plugin.common.PluginRegistry.Registrar registrar) {
       // Removed - registration now happens via FlutterPlugin interface methods
    }
    */
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
        ...
        // Register global callbacks (safe to do here if they don't rely on Activity)
        registerGlobalCallbacks();
    }

    @Override
    public void onDetachedFromEngine(@NonNull FlutterPluginBinding binding) {
        Log.d(LIBRARY_NAME, "FFmpegKitFlutterPlugin onDetachedFromEngine.");
        uninitChannels();
        this.context = null;
    }
    ...
    public void onDetachedFromActivity() {
        ...
        uninitActivityRelated();
    }

    @Override
    public void onReattachedToActivityForConfigChanges(@NonNull ActivityPluginBinding binding) {
        Log.d(LIBRARY_NAME, "FFmpegKitFlutterPlugin onReattachedToActivityForConfigChanges.");
        onAttachedToActivity(binding); // Re-attach listeners and context
    }

     @Override
     public void onDetachedFromActivityForConfigChanges() {
         Log.d(LIBRARY_NAME, "FFmpegKitFlutterPlugin onDetachedFromActivityForConfigChanges.");
         onDetachedFromActivity(); // Clean up activity-related things
     }
    ...
    // PluginRegistry.ActivityResultListener method
    @Override
    public boolean onActivityResult(final int requestCode, final int resultCode, @Nullable final Intent data) {
        ...
        // --- Ensure lastInitiatedIntentResult is available ---
        final Result resultToUse = lastInitiatedIntentResult;
        if (resultToUse == null) {
             Log.w(LIBRARY_NAME, String.format("Ignored activity result for requestCode: %d because lastInitiatedIntentResult is null.", requestCode));
             return false; // Can't handle without the result callback reference
        }
        lastInitiatedIntentResult = null; // Consume it

        if (requestCode == READABLE_REQUEST_CODE || requestCode == WRITABLE_REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                if (data == null) {
                    resultHandler.successAsync(resultToUse, null);
                } else {
                    final Uri uri = data.getData();
                    resultHandler.successAsync(resultToUse, uri == null ? null : uri.toString());
                }
            } else {
                resultHandler.errorAsync(resultToUse, "SELECT_CANCELLED", String.valueOf(resultCode));
            }
            return true;
        } else {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        // Keep the existing onMethodCall implementation exactly as it was
        // It uses resultHandler, which handles threading correctly.
        // --- Existing switch statement logic here ---
    ...
    // --- V2 EMBEDDING CHANGE: init method removed ---
    /*
    @SuppressWarnings("deprecation")
    protected void init(final BinaryMessenger messenger, final Context context, final Activity activity, final io.flutter.plugin.common.PluginRegistry.Registrar registrar, final ActivityPluginBinding activityBinding) {
      // Removed - logic moved to onAttachedToEngine and onAttachedToActivity
    }
    */

    // --- V2 EMBEDDING CHANGE: Separated cleanup logic ---
    protected void uninitActivityRelated() {
         if (this.activityPluginBinding != null) {
             // Remove activity-specific listeners
             this.activityPluginBinding.removeActivityResultListener(this);
         }
         // Null out activity-specific references
         this.activity = null;
         this.activityPluginBinding = null;
         this.lastInitiatedIntentResult = null; // Clear dangling result if activity is destroyed
         Log.d(LIBRARY_NAME, "FFmpegKitFlutterPlugin Activity related resources uninitialized.");
    }

    protected void uninitChannels() {
         if (methodChannel != null) {
            methodChannel.setMethodCallHandler(null);
            methodChannel = null;
             Log.d(LIBRARY_NAME, "FFmpegKitFlutterPlugin Method channel uninitialized.");
         }
         if (eventChannel != null) {
            eventChannel.setStreamHandler(null);
            eventChannel = null;
            Log.d(LIBRARY_NAME, "FFmpegKitFlutterPlugin Event channel uninitialized.");
         }
    }


    // --- Keep the rest of the helper/implementation methods ---
    // (e.g., abstractSession* methods, getArch, ffmpegSession*, ffprobeSession*,
    //  mediaInformationSession*, etc. They should not need changes as they
    //  don't directly use the Registrar or plugin binding.)
    // ... All existing protected helper methods like abstractSessionGetEndTime, ... toMap, toList, etc. ...
     ...
     protected void enableLogs(@NonNull final Result result) {
         enableLogs(); // Calls the local helper method

         resultHandler.successAsync(result, null);
     }

     protected void disableLogs(@NonNull final Result result) {
         disableLogs(); // Calls the local helper method

         resultHandler.successAsync(result, null);
     }

     protected void enableStatistics(@NonNull final Result result) {
         enableStatistics(); // Calls the local helper method

         resultHandler.successAsync(result, null);
     }

     protected void disableStatistics(@NonNull final Result result) {
         disableStatistics(); // Calls the local helper method
     ...
     protected void setFontDirectory(@NonNull final String fontDirectoryPath, @Nullable final Map<String, String> fontNameMapping, @NonNull final Result result) {
         // Use the application context obtained in onAttachedToEngine
         if (context != null) {
             FFmpegKitConfig.setFontDirectory(context, fontDirectoryPath, fontNameMapping);
             resultHandler.successAsync(result, null);
         } else {
             Log.w(LIBRARY_NAME, "Cannot setFontDirectory. Context is null. Was the plugin attached to the engine?");
     ...
     protected void setFontDirectoryList(@NonNull final List<String> fontDirectoryList, @Nullable final Map<String, String> fontNameMapping, @NonNull final Result result) {
         if (context != null) {
             ...
             Log.w(LIBRARY_NAME, "Cannot setFontDirectoryList. Context is null. Was the plugin attached to the engine?");
             resultHandler.errorAsync(result, "INVALID_CONTEXT", "Context is null.");
         }
     }
     ...
     protected void registerNewFFmpegPipe(@NonNull final Result result) {
         if (context != null) {
             ...
             Log.w(LIBRARY_NAME, "Cannot registerNewFFmpegPipe. Context is null. Was the plugin attached to the engine?");
             resultHandler.errorAsync(result, "INVALID_CONTEXT", "Context is null.");
         }
     }
     ...
     protected void selectDocument(@NonNull final Boolean writable, @Nullable final String title, @Nullable final String type, @Nullable final String[] extraTypes, @NonNull final Result result) {
         ...
         if (context != null) {
             if (activity != null) {
                 try {
                     lastInitiatedIntentResult = result; // Store the Result callback
                     ...
                     lastInitiatedIntentResult = null; // Clear if startActivityForResult failed
                 }
             } else {
     ...
     // --- Local State flags (keep as they were) ---
     ...
     // --- Keep all static helper methods (toInt, toMap, toList, etc.) ---
    ...
    protected static Map<String, Object> toMap(final Statistics statistics) {
        ...
        if (statistics != null) {
            ...
            // Handle potential Long overflow for Flutter number types if needed, casting to int here
            long size = statistics.getSize();
            statisticsMap.put(KEY_STATISTICS_SIZE, (size > Integer.MAX_VALUE || size < Integer.MIN_VALUE) ? -1 : (int) size); // Or handle differently
     ...
     protected static Map<String, Object> toMap(@Nullable final MediaInformation mediaInformation) {
        final HashMap<String, Object> map = new HashMap<>();
        if (mediaInformation != null && mediaInformation.getAllProperties() != null) {
            JSONObject allProperties = mediaInformation.getAllProperties();
             // Defensive copy in case the original JSON object is mutated elsewhere (unlikely but safe)
             try {
                  allProperties = new JSONObject(allProperties.toString());
             } catch(JSONException e){
                 // Should not happen if input is valid JSON, log if needed
             }
             Iterator<String> keys = allProperties.keys();
             while (keys.hasNext()) {
                 String key = keys.next();
                 Object value = jsonValueToJavaObject(allProperties.opt(key)); // Recursively convert JSON types
                 map.put(key, value);
             }
         }
         return map; // Return empty map if null or no properties
     }


    // Helper for converting JSONObject properties to standard Java maps recursively
     @Nullable
     private static Object jsonValueToJavaObject(@Nullable Object jsonValue) {
         if (jsonValue instanceof JSONObject) {
             ...
             // Primitive types (String, Number, Boolean) are usually fine directly
             return jsonValue;
         }
     }

     // Helper to convert JSONObject to Map<String, Object>
     @NonNull
     private static Map<String, Object> jsonObjectToMap(@NonNull JSONObject jsonObject) {
         ...
         while (keys.hasNext()) {
             String key = keys.next();
             Object value = jsonValueToJavaObject(jsonObject.opt(key)); // Recursive call
             map.put(key, value);
         }
         return map;
     }

     // Helper to convert JSONArray to List<Object>
     @NonNull
     private static List<Object> jsonArrayToList(@NonNull JSONArray jsonArray) {
         final List<Object> list = new ArrayList<>();
         for (int i = 0; i < jsonArray.length(); i++) {
             list.add(jsonValueToJavaObject(jsonArray.opt(i))); // Recursive call
         }
         return list;
     }
    ...
    protected static List<Map<String, Object>> toSessionMapList(final List<? extends Session> sessionList) {
        ...
        if (sessionList != null) {
            for (int i = 0; i < sessionList.size(); i++) {
                final Map<String, Object> sessionMap = toMap(sessionList.get(i));
                if (sessionMap != null) list.add(sessionMap);
            }
        }
        return list;
    }
        ...
        final List<Map<String, Object>> list = new ArrayList<>();
         ...
         if (logList != null) {
             for (int i = 0; i < logList.size(); i++) {
                 final Map<String, Object> logMap = toMap(logList.get(i));
                 if(logMap != null) list.add(logMap);
             }
         }
        return list;
    }
    ...
    protected static List<Map<String, Object>> toStatisticsMapList(final List<com.arthenica.ffmpegkit.Statistics> statisticsList) {
        ...
        if (statisticsList != null) {
            for (int i = 0; i < statisticsList.size(); i++) {
                final Map<String, Object> statsMap = toMap(statisticsList.get(i));
                 if(statsMap != null) list.add(statsMap);
            }
        }
        return list;
    }

    protected static boolean isValidPositiveNumber(@Nullable final Integer value) {
        return (value != null) && (value >= 0);
    }

    // Methods to emit events back to Flutter
    protected void emitLog(final com.arthenica.ffmpegkit.Log log) {
        if (eventSink != null) {
            final HashMap<String, Object> logMap = new HashMap<>();
            logMap.put(EVENT_LOG_CALLBACK_EVENT, toMap(log));
            resultHandler.successAsync(eventSink, logMap); // Use handler to ensure main thread
        }
    }
    ...
    protected void emitStatistics(final Statistics statistics) {
       if (eventSink != null) {
           ...
           resultHandler.successAsync(eventSink, statisticsMap); // Use handler to ensure main thread
       }
    }
    ...
    protected void emitSession(final Session session) {
        if (eventSink != null) {
            ...
            resultHandler.successAsync(eventSink, sessionMap); // Use handler to ensure main thread
        }
    }
